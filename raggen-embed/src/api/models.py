...
class ParagraphOptions(BaseModel):
    """Options for paragraph processing."""
    enabled: bool = Field(
        default=False,
        description="Whether to use paragraph processing"
    )
    max_length: Optional[int] = Field(
        default=None,
        ge=100,
        le=5000,
        description="Maximum length of a paragraph (100-5000 characters)"
    )
    min_length: Optional[int] = Field(
        default=None,
        ge=50,
        le=1000,
        description="Minimum length of a paragraph (50-1000 characters)"
    )
    ...
    def validate_merge_strategy(cls, v):
        ...
        if v is not None and v not in ["mean", "weighted"]:
            raise ValueError("Merge strategy must be either 'mean' or 'weighted'")
        return v
...
# Update TextRequest to include paragraph options
TextRequest.model_fields["paragraph_options"] = Field(
    default=None,
    description="Options for paragraph processing"
)
TextRequest.model_config["json_schema_extra"]["example"]["paragraph_options"] = {
    "enabled": True,
    "max_length": 1000,
    "min_length": 100,
    "overlap": 100,
...
BatchTextRequest.model_fields["paragraph_options"] = Field(
    default=None,
    description="Options for paragraph processing"
)
BatchTextRequest.model_config["json_schema_extra"]["example"]["paragraph_options"] = {
    "enabled": True,
    "max_length": 1000,
    "min_length": 100,
    "overlap": 100,
